...
public class AllVersusAllCompetition {
    ...
    public void runDuelSets(int count) {
    ...
    public void runDuelSet() {
        ...
        for (AbstractStrategy s1 : strategies) {
            for (AbstractStrategy s2 : strategies) {
                duels.add(new MessyDuel(0.05f, 200, s1, s2));
            }
        }
        ...
        for (Duel d : duels) {
            ...
            String duelTitle = String.format(duelNameFormat, d.strategy0.getName(), d.strategy1.getName(), d.getName());
